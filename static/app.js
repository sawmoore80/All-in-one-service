...
async function register(){const email=$('email').value,password=$('password').value;const d=await call('/api/register',{method:'POST',body:JSON.stringify({email,password})}); if(d.ok){toast('Account created'); showApp();} else {toast(d.error||'Error');}}
async function login(){const email=$('email').value,password=$('password').value;const d=await call('/api/login',{method:'POST',body:JSON.stringify({email,password})}); if(d.ok){toast('Signed in'); showApp();} else {toast(d.error||'Error');}}
async function logout(){await call('/api/logout'); location.reload();}

function showApp(){ $('auth').style.display='none'; document.body.insertAdjacentHTML('afterbegin','<div class=\"container\" id=\"root\"></div>'); const root=$('root'); root.replaceWith(document.querySelector('.container')||document.body); $('app').style.display='block'; loadAll(); }
...
function insightCard(i){const sel=selectedForPlan.has(i.id);return `<div class="card" style="margin:10px 0"><div style="display:flex;justify-content:space-between;gap:8px"><div><div style="font-weight:800">${i.title}</div><div class="sub">Campaign: ${i.campaign_name}</div></div><div>${badge(i.kpi,'kpi')} ${badge(i.severity,i.severity)} ${badge('Score '+i.priority_score,'low')}</div></div><div style="margin-top:8px"><b>Why:</b><pre>${JSON.stringify(i.evidence||{},null,2)}</pre></div><div><b>Actions (48h):</b><ul>${(i.actions||[]).map(a=>`<li>${a}</li>`).join('')}</ul></div><div class="sub" style="margin-top:6px">Expected impact: +${Math.round((i.expected_impact||0)*100)}%</div><div style="margin-top:10px"><button class="btn ${sel?'danger':''}" onclick="toggleSelect(${i.id})">${sel?'Remove':'Add'} to Plan</button></div></div>`;}
...
async function loadPosts(){try{const d=await call('/api/posts');$('posts').innerHTML=(d.posts||[]).map(p=>`<div class="badge">${p.platform}</div> ${p.title||''} <span class="sub">· ${p.caption||''}</span>`).join('<br>')||'<div class="sub">No posts yet.</div>';}catch(e){showOut(e);}}
...
async function askAI(){const q=$('aiq').value||'';const d=await call('/api/ai/ask',{method:'POST',body:JSON.stringify({q})});$('aiout').textContent=d.answer||JSON.stringify(d,null,2);}

async function connect(platform){const d=await call(`/api/oauth/${platform}`); if(d.auth_url){window.open(d.auth_url,'_blank'); toast('Opened auth (demo URL)');} else {toast('OAuth error');}}
async function loadConnections(){try{const d=await call('/api/social/connections');$('connections').innerHTML=(d.connections||[]).map(c=>`<span class="badge ${c.connected?'low':''}">${c.platform}${c.connected?' · connected':''}</span>`).join(' ')||'<span class="sub">No connections</span>';}catch(e){showOut(e);}}

async function loadAll(){ await Promise.allSettled([refreshHealth(),loadKPIs(),loadTrends(),loadRecs(),loadConnections(),loadPosts()]); }
document.addEventListener('DOMContentLoaded', ()=>{ /* show auth by default */ });

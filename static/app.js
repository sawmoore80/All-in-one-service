(function(){
  ...
  function call(path, opts){
    ...
    return fetch(path,base).then(function(r){ return r.text().then(function(txt){ var d; try{d=JSON.parse(txt);}catch(_){d={raw:txt}}; if(!r.ok) throw d; return d;}); });
  }
  ...
  function refreshAuthBadge(){
    return call('/api/me').then(function(d){
      $('authBadge').textContent = d.auth ? (d.email||'User') : 'Guest';
      var w=$('whoami'); if(w) w.textContent = d.auth ? (d.email||'user') : 'guest';
  ...
  function renderKPIs(k){
    var items=[{label:'Total Spend',value:currency(k.total_spend)},{label:'Avg ROAS',value:(Number(k.avg_roas)||0).toFixed(2)},{label:'Avg CPA',value:currency(k.avg_cpa)},{label:'Conversions',value:(Number(k.conversions)||0).toLocaleString()}];
  ...
  function insightCard(i){
    ...
    return '<div class="card" style="margin:10px 0">'+
      '<div style="display:flex;justify-content:space-between;gap:8px"><div><div style="font-weight:800">'+i.title+'</div><div class="sub">Campaign: '+i.campaign_name+'</div></div><div>'+badge(i.kpi,'kpi')+' '+badge(i.severity,i.severity)+' '+badge('Score '+i.priority_score,'low')+'</div></div>'+
  ...
  function renderInsights(list){
    ...
    for(var i=0;i<buttons.length;i++){
      buttons[i].addEventListener('click',function(ev){
        ...
        if(selectedForPlan.has(id)){selectedForPlan.delete(id);toast('Removed');} else {selectedForPlan.add(id);toast('Added');}
        loadRecs();
      });
    }
  }
  ...
  function loadPosts(){return call('/api/posts').then(function(d){$('posts').innerHTML=(d.posts||[]).map(function(p){return '<div class="badge">'+p.platform+'</div> '+(p.title||'')+' <span class="sub">· '+(p.caption||'')+'</span>';}).join('<br>')||'<div class="sub">No posts yet.</div>';}).catch(function(){ $('posts').innerHTML='<div class="sub">No posts yet.</div>';});}
  ...
  // ---------- FEATURES ----------
  function generatePlaybook(){var body=selectedForPlan.size?{insight_ids:Array.from(selectedForPlan)}:{};return call('/api/playbook',{method:'POST',body:JSON.stringify(body)}).then(function(d){showOut(d);renderPlan(d.plan||[]);showModal();}).catch(showOut);}
  function askAI(){var q=$('aiq').value||'';return call('/api/ai/ask',{method:'POST',body:JSON.stringify({q:q})}).then(function(d){$('aiout').textContent=d.answer||JSON.stringify(d,null,2);}).catch(function(e){$('aiout').textContent='AI error';showOut(e);});}
  function connect(platform){return call('/api/oauth/'+platform).then(function(d){if(d.auth_url){window.open(d.auth_url,'_blank');toast('Opened auth (demo URL)');}else{toast('OAuth error');}}).catch(function(){toast('Sign in to connect');});}
  function loadConnections(){return call('/api/social/connections').then(function(d){$('connections').innerHTML=(d.connections||[]).map(function(c){return '<span class="badge '+(c.connected?'low':'')+'">'+c.platform+(c.connected?' · connected':'')+'</span>';}).join(' ')||'<span class="sub">No connections</span>';}).catch(function(){$('connections').innerHTML='<span class="sub">Sign in to view connections.</span>';});}

  // ---------- CONTACT ----------
  function submitContact(){
    var payload={name:($('c_name')||{}).value||'',email:($('c_email')||{}).value||'',company:($('c_company')||{}).value||'',phone:($('c_phone')||{}).value||'',message:($('c_msg')||{}).value||''};
  ...
  // ---------- WIRE + EXPORT ----------
  function wire(){
    // keep IDs wiring in case onclick is missing
    [['btnAccount',openAuth],['btnCloseAuth',closeAuth],['btnDoRegister',register],['btnDoLogin',login],['btnLogout',logout],
     ...
     ['btnAskAI',askAI],['btnIG',function(){connect('instagram')}],['btnTT',function(){connect('tiktok')}],
     ['btnFB',function(){connect('facebook')}],['btnYT',function(){connect('youtube')}],
     ['btnContact',submitContact],['btnClosePlan',hideModal]
    ].forEach(function(p){var b=$(p[0]); if(b) b.addEventListener('click', p[1]);});
  }
  ...
  // export for inline onclick fallbacks
  ...
  window.generatePlaybook=generatePlaybook; window.clearSelection=clearSelection;
  window.askAI=askAI; window.connect=connect; window.submitContact=submitContact; window.hideModal=hideModal;
})();
